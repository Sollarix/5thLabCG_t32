...
public:
    ...
    bool Init()
    {
        ...
        if (!m_pMesh1->LoadMesh("F:/Content/phoenix_ugv.md2")) {
            return false;
        }

        m_pMesh2 = new Mesh();

        if (!m_pMesh2->LoadMesh("F:/Content/jeep.obj")) {
            return false;
        }

        m_pMesh3 = new Mesh();

        if (!m_pMesh3->LoadMesh("F:/Content/hheli.obj")) {
            return false;
        }
...
int main(int argc, char** argv)
{
    ...
    if (!GLUTBackendCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, 32, false, "Lab 5 Task 32")) {
        return 1;
    }
